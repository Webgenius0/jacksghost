...
export default function ImportAgentModal({ isOpen, onClose }: Props) {
    const [file, setFile] = useState<File | null>(null);
    const [isDragging, setIsDragging] = useState(false);
    const [defaultStatus, setDefaultStatus] = useState<string>('approved');
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        router.post(route('agents.import'), formData, {
            forceFormData: true,
            preserveScroll: true,
            onSuccess: (page) => {
                setIsSubmitting(false);
                ...
                if (flash?.import_errors && flash.import_errors.length > 0) {
                    toast.warning(`${flash.import_errors.length} rows had warnings. Check notices.`);
                }
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <DialogDescription className="text-sm text-muted-foreground mt-0.5">
                                Upload an Excel (.xlsx, .xls) or CSV spreadsheet to batch import agents.
                            </DialogDescription>
                        </div>
                    </div>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div>
                        ...
                        {!file ? (
                            ...
                            >
                                <div className="p-3 rounded-full bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 mb-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {/* Default Status */}
                        <div>
                            <Label htmlFor="default_status" className="text-sm font-medium mb-1.5 block">
                                Default Status
                            </Label>
                            <select
                                id="default_status"
                            ...
                            <p className="text-[11px] text-muted-foreground mt-1">
                                Applied when a row has no status specified.
                            </p>
                        </div>
                            ...
                            <div className="flex items-start space-x-2 pt-1.5">
                                ...
                                <div className="grid gap-0.5 leading-none">
                                    ...
                                    <p className="text-[11px] text-muted-foreground mt-0.5">
                                        Matches on Email or Agent Name.
                                    </p>
                                </div>
                            </div>
                        </div>
                            ...
                            {showColumnsGuide && (
                                <div className="mt-2 text-xs text-muted-foreground space-y-1.5 bg-background/80 p-2.5 rounded border border-border/50">
                                    <p>
                                        ...
                                        <code className="text-[11px] bg-muted px-1 py-0.5 rounded">Agent Name</code>
                                    </p>
                                    <p>
                                        <span className="font-semibold text-foreground">Optional:</span>{' '}
                                        Agency Name, Email, Phone Number, Address, Website, Institution, Degree, Graduation Year, Notable Clients (comma separated), Services (comma separated), Background Info, Status (approved / pending / rejected)
                                    </p>
                                </div>
                            )}
                        </div>
